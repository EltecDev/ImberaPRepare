...
public class GetHexFromRealDataImbera {
    ...
    public static String calculateChacksumString(String data) {
        int checksum = 0;
        //String s;
        /*StringBuilder stringBuilder = new StringBuilder();
        for (String dato : data) {
        ...
        //s = stringBuilder.toString().trim();
        for (int h = 0; h < data.length(); h += 2) {
            if (h + 2 >= data.length()) {
                checksum = checksum + getDecimal(data.substring(h));
            } else
                checksum = checksum + getDecimal(data.substring(h, h + 2));
        }
        String c = Integer.toHexString(checksum);
        if (c.length() == 1)
            c = "0000000" + c;
